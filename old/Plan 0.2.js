...
*
*@param {Range[]} rawForecast - A two-dimensional array of [Dates][Projected Sales].
*@param {Range[]} products - vector of product information. Columns: Starting Inventory, Average Qty Per Sale, Average Cost, lead Time, Minimum Order Quantity, Current PO Quantity, and Time Periods Until Current PO is Receieved.
...
*@version 0.3 - Chinese New Year Adjustment added 12/8/2017
*/
/*  function PLAN(rawForecast,product,outputType){
    ...
    for(t=0;t<numTimes;t++){
      salesSum += rawForecast[1][t];
    }
    
    //Safety stock calculation: 30% of leadtime x average daily projected sales over full forecast
    safetyStock = (salesSum/numTimes)*leadTime*0.3;
    ...
    for(t=0;t<numTimes;t++){
      ...
      // Check for stockout
      if(inventory[t] < safetyStock){
        if(t-leadTime < 0){
          if(Math.abs(inventory[t]) > moq){
            orders[0] += Math.abs(inventory[t])+safetyStock; //Order up to safety stock level
          }
          else {
          ...
          if(Math.abs(inventory[t]) > moq){
            //CNY
            if(monthTest(rawForecast[0][t-leadTime])==="2"){
              orders[t - leadTime - 28] += Math.abs(inventory[t])+safetyStock; //Order up to safety stock level
              t = t - leadTime - 29; //Move back to the most recent purchase on the next iteration.
            }
            else {
              orders[t-leadTime] += Math.abs(inventory[t])+safetyStock; //Order up to safety stock level
            ...
            //CNY
            if(monthTest(rawForecast[0][t-leadTime])==="2"){
              orders[t - leadTime - 28] += moq;  //Order the MOQ
              t = t - leadTime - 29; //Move back to the most recent purchase on the next iteration.
            }
            else {
